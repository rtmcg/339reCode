...
while dataRead == False:
    serialPort.write(chr(value).encode(encoding = 'latin-1'))
    t.sleep(0.1)
    ...
    while (inByte > 0) & (byteCount < arraySize):
        ...
        byteCount = byteCount + 1 # could use byteCount += 1
        data = data + [dataByte] # could use data += [dataByte] or data.extend(dataByte) also (or option in other file)
